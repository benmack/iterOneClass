...
one_step_occ <- function(sttngs, iter, filename_U, test_set,
  ...
  if (!file.exists(fname_model)) {
    ...
    pdf(file=gsub("WHAT", "model_grid_puF", fname_plot))
      trellis.par.set(caretTheme())
      p = plot(model, plotType="level", metric="puF")
      print(p)
      dev.off()
    pdf(file=gsub("WHAT", "model_grid_puAuc", fname_plot))
      trellis.par.set(caretTheme())
      p = plot(model, plotType="level", metric="puAuc")
      print(p)
    dev.off()
